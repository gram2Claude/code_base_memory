...
description: >
  ...
  удалить окончательно (с подтверждением); update — переиндексация; reapply — обновить
  регистрацию (хуки/инструкции) без переиндексации. Запускать в корне проекта.
...
argument-hint: "[on|off|clear [--hard]|update|reapply|status]"
---
...
- (пусто) или `on` → **on**; `off` → **off**; `clear` → **clear**; `clear --hard` → **clear --hard**; `update` → **update**; `reapply` → **reapply**; `status` → **status**.
...
### reapply — обновить регистрацию без переиндексации (донести апдейт скила)

Назначение: после обновления скила (новый matcher PostToolUse, новый блок CLAUDE.md и т.п.)
донести свежие хуки/инструкции до УЖЕ активированного проекта, НЕ перестраивая индекс
(нет DB-lock — можно даже при открытой сессии; новые хуки/блок подхватятся следующей сессией).

1. Выполнить: `<MC> -Mode reapply -Project "<PROJECT>"`. Движок требует существующий
   `.ontoindex/` (иначе просит сначала `on`), затем делает то же, что `on`, но БЕЗ `analyze`:
   in-place апгрейд matcher наших хуков, перевставка блока MEMORY_CODE из текущего шаблона,
   merge `.mcp.json`, дозапись `.gitignore`/`.ontoindexignore`. Индекс и БД не трогаются.
2. Сказать: «Регистрация обновлена без переиндексации; новые хуки/инструкции — со следующей сессии».
